"""
Targeted NCUC portal search for high-quality DEP documents to fill gaps.

Focuses on families with the fewest extracted charges and targets specific
compliance and annual adjustment dockets.
"""
import json
...
# DEP families with gaps (fewest charges extracted)
# and their known NCUC compliance/annual adjustment dockets
DEP_GAP_TARGETS = [
    # Priority order: families already searched but with better sources expected
    {
    ...
    },
    {
        ...
        "leaf": "607",
        "keywords": ["leaf 607", "STS", "storm securitization", "annual"],
        "priority": 2,
    },
    {
    ...
    # No sources found yet - need more targeted searches
    {
        ...
        for docket_str in dockets:
            ...
            for doc in docs:
                ...
                if is_tariff or "tariff" in title or "compliance" in title:
                    ...
                    print("      [FOUND]")  # ASCII equivalent for checkmark

finally:
    close_authenticated_context(pw, ctx)

# Save results
out_path = Path("data/dep_gap_search_results.json")
